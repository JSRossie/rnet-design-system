...
CSS_SRC=(tokens.css colors_and_type.css glyphs.css controls.css markdown.css)
...
# @import lines (every dependency is already inlined above its dependents).
{
  ...
  echo "   Generated by cdn/build.sh: tokens + colors_and_type + glyphs +"
  echo "   controls + markdown, concatenated. One <link>, no @import waterfall. */"
  for f in "${CSS_SRC[@]}"; do
    echo ""
    echo "/* ===== $f ===== */"
    grep -vE '^[[:space:]]*@import[[:space:]]+"(tokens|colors_and_type|glyphs|controls|markdown)\.css"' "$f"
  done
} > "$CV/css/v1/rnet.css"

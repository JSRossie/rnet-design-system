...
The R—Net logo lockups plus the parent JR monogram (27 variants × svg + png).
The lockup SVGs are **outlined** — the wordmark text is baked to `<path>`, so
they carry no font dependency; the `jr-monogram-{day,night,mono}` files are
path-only to begin with. All render identically as an `<img>`, CSS background, or
design-tool import. Prefer the SVG; the PNG is for raster contexts (email, docs).
...
`png/`, regenerated by `components/rnet/build-logos.sh`); the monogram's own
source stays at `components/monogram-{light,dark,mono}.svg`, which the same
script publishes into `svg/`+`png/`. `cdn/build.sh` copies `svg/` + `png/` into
`logos/` at deploy, so the CDN carries no hand-copied duplicate — the published
copy is `.gitignored`, exactly like the `css/v1/` channel.
...
- **Logos:** edit `components/rnet/src/` (lockups) or
  `components/monogram-*.svg` (the JR seal), run
  `components/rnet/build-logos.sh`, commit the regenerated
  `components/rnet/{svg,png}`, push. `build.sh` copies them into `logos/` on
  deploy — nothing to commit under `cdn/`.
...
curl -sI https://cdn.rossie.net/charts-and-vectors/logos/svg/jr-monogram-day.svg
```
